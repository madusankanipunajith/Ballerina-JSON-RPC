...
class EmpMethod{
    *server:JRPCMethods;
    ...
    public function getLastName(types:InputParams params) returns error|string{
        Person person = check params.cloneWithType();
        string l_name = from var {firstName, lastName} in employees where firstName == person.first select lastName;
        return l_name;   
    }
